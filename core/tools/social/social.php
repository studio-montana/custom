 ...
 * WIDGETS
 */
require_once (CUSTOM_PLUGIN_PATH.'/'.CUSTOM_PLUGIN_TOOLS_FOLDER.SOCIAL_TOOL_NAME.'/widgets/tool-social-widget.class.php');
...
function tool_social_custom_front_enqueue_styles_tools($dependencies) {
	...
	$css_social = locate_web_ressource(CUSTOM_PLUGIN_TOOLS_FOLDER.SOCIAL_TOOL_NAME.'/css/tool-social.css');
	if (!empty($css_social))
...
function tool_social_custom_admin_enqueue_styles_tools($dependencies) {
	...
	$css_social = locate_web_ressource(CUSTOM_PLUGIN_TOOLS_FOLDER.SOCIAL_TOOL_NAME.'/css/tool-social-admin.css');
	if (!empty($css_social))
		wp_enqueue_style('tool-social-css', $css_social, $dependencies, '1.0');
}
