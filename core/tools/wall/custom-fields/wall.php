...
function wall_save_post($post_id){
	...
	if (in_array(get_post_type($post_id), $available_posttypes)){
		...
		if (!empty($_POST[META_WALL_DISPLAY_PRESENTATION_MASONRY_WIDTH_CUSTOMIZED])){
			update_post_meta($post_id, META_WALL_DISPLAY_PRESENTATION_MASONRY_WIDTH_CUSTOMIZED, sanitize_text_field($_POST[META_WALL_DISPLAY_PRESENTATION_MASONRY_WIDTH_CUSTOMIZED]));
		}else{
			update_post_meta($post_id, META_WALL_DISPLAY_PRESENTATION_MASONRY_WIDTH_CUSTOMIZED, "250");
		}
