 ...
 * REQUIREMENTS
*/
require_once (CUSTOM_PLUGIN_PATH.'/'.CUSTOM_PLUGIN_TOOLS_FOLDER.DISPLAY_TOOL_NAME.'/custom-fields/display.php');
...
* @param string $after
*/
function custom_display_title($post_id = null, $display = true, $test_hide_property = true, $before = '<h1 class="entry-title">', $after = '</h1>'){
	$res = '';
	if (!empty($post_id))
		$post_id = get_the_ID();
	$hide_title = 'off';
...
if (!function_exists("custom_display_subtitle")):
/**
 * display post subtitle
* @param string $before
* @param string $after
*/
function custom_display_subtitle($post_id = null, $display = true, $before = '<h2 class="entry-subtitle">', $after = '</h2>'){
	$res = '';
	if (!empty($post_id))
		$post_id = get_the_ID();
...
* @param string $after
*/
function custom_display_thumbnail($post_id = null, $size = 'post-thumbnail', $attr = '' , $display = true, $test_hide_property = true, $before = '<div class="entry-thumbnail">', $after = '</div>'){
	$res = '';
	if (!empty($post_id))
		$post_id = get_the_ID();
...
* @param string $after
*/
function custom_display_badge($post_id = null, $display = true, $before = '<div class="has-badge"><span>', $after = '</span></div>'){
	$res = '';
	if (!empty($post_id))
		$post_id = get_the_ID();
