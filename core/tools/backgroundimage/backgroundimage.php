 ...
 * REQUIREMENTS
*/
require_once (CUSTOM_PLUGIN_PATH.'/'.CUSTOM_PLUGIN_TOOLS_FOLDER.BACKGROUNDIMAGE_TOOL_NAME.'/custom-fields/backgroundimage.php');
require_once (CUSTOM_PLUGIN_PATH.'/'.CUSTOM_PLUGIN_TOOLS_FOLDER.BACKGROUNDIMAGE_TOOL_NAME.'/inc/customizer.php');
...
if (!function_exists("custom_backgroundimage")):
/**
...
*/
function custom_backgroundimage($auto_insert = false){
	$id_post = null;
	if (is_home()){ // blog page
		$id_post = get_option('page_for_posts');
	}else if (!is_404()){
...
if (!function_exists("custom_backgroundimage_autoinsert")):
/**
 * Display background image/color/opacity
* @param boolean $auto_insert : boolean - do not specify true if you use this method manualy in your template theme
*/
...
add_action('wp_footer', 'custom_backgroundimage_autoinsert');
endif;
