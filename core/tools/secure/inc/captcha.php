...
function secure_captcha_generate_field($field_name){
	$field = "";
	$is_validated = custom_session_get($field_name.'-validate', "");
	$old_result = custom_session_get($field_name.'-result', "");
	if ($is_validated == 1 || empty($old_result)){
		$captcha_1 = rand(1, 15);
		$captcha_2 = rand(1, 15);
		custom_session_set($field_name.'-result', $captcha_1 + $captcha_2);
		custom_session_set($field_name.'-value-1', $captcha_1);
		custom_session_set($field_name.'-value-2', $captcha_2);
		custom_session_set($field_name.'-validate', 0);
	}else{
		$captcha_1 = custom_session_get($field_name.'-value-1');
		$captcha_2 = custom_session_get($field_name.'-value-2');
	...
	$error = custom_session_get($field_name.'-error', "");
	custom_session_set($field_name.'-error', ""); // clear captcha error in session (otherwise, captcha-error appear when form is not submited)
	if (!empty($error)){
