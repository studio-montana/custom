	...
	<header class="entry-header">
		...
		<h1 class="entry-title">
			<a href="<?php the_permalink(); ?>" rel="bookmark">
				<?php if (function_exists("custom_display_title")) custom_display_title(get_the_ID(), true, false, '', ''); else the_title(); ?>
			</a>
		...
		if (get_post_type() == 'event'){
			...
			if ($meta_date_begin_s == $meta_date_end_s){
				...
				if ($meta_year_begin_s == $meta_year_end_s){
					$date_s = $meta_day_begin_s." ".$meta_month_begin_s." ".__("to", CUSTOM_PLUGIN_TEXT_DOMAIN)." ".$meta_day_end_s." ".$meta_month_end_s." ".$meta_year_end_s;
				}else{
					$date_s = $meta_day_begin_s." ".$meta_month_begin_s." ".$meta_year_begin_s." ".__("to", CUSTOM_PLUGIN_TEXT_DOMAIN)." ".$meta_day_end_s." ".$meta_month_end_s." ".$meta_year_end_s;
				}
			}
		}
		if (!empty($date_s)){
	...
	<footer class="entry-meta">
		<?php if (comments_open() && ! is_single()) : ?>
			<div class="comments-link">
				<?php custom_comments_popup_link('<span class="leave-reply">' . __('Comment', CUSTOM_PLUGIN_TEXT_DOMAIN) . '</span>', __('One comment', CUSTOM_PLUGIN_TEXT_DOMAIN), __('See % comments', CUSTOM_PLUGIN_TEXT_DOMAIN) ); ?>
			</div><!-- .comments-link -->
