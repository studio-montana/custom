	...
	if (!empty($events)){
		...
		<ul class="list-events">
			...
			foreach ($events as $post){
				...
				<li class="event">
					<?php 
					$template = locate_ressource(CUSTOM_PLUGIN_TOOLS_FOLDER.EVENT_TOOL_NAME.'/widgets/templates/tool-event-widget-output-event.php');
					if (!empty($template))
						include($template);
					?>
				</li>
		...
		?>
		<div class="no-content"><?php _e("No upcoming event", CUSTOM_PLUGIN_TEXT_DOMAIN); ?></div>
		<?php
	}	
	?>
